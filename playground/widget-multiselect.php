...
echo "\n--- Multiselect: basic ---\n";
$r = Prompty::multiselect('Features', options: ['ts' => 'TypeScript', 'eslint' => 'ESLint', 'prettier' => 'Prettier']);
echo '  Result: ' . ($r !== NULL ? (count($r) > 0 ? implode(', ', $r) : 'none') : 'cancelled') . "\n";

echo "\n--- Multiselect: with description ---\n";
$r = Prompty::multiselect('CI checks',
  options: ['lint' => 'Linting', 'test' => 'Unit tests', 'e2e' => 'E2E tests', 'build' => 'Build verification'],
  description: "Select checks to run in CI.\nSpace to toggle, enter to confirm.",
);
echo '  Result: ' . ($r !== NULL ? (count($r) > 0 ? implode(', ', $r) : 'none') : 'cancelled') . "\n";

echo "\n--- Multiselect: with hints ---\n";
$r = Prompty::multiselect('Integrations',
  options: ['sentry' => 'Sentry', 'analytics' => 'Analytics', 'cdn' => 'CDN'],
  hints: [
...
$r = Prompty::multiselect('Dev tools',
  options: ['phpcs' => 'PHPCS', 'phpstan' => 'PHPStan', 'rector' => 'Rector'],
  default: ['phpcs', 'phpstan', 'rector'],
